...
if (array_key_exists('action', $_GET) && array_key_exists('id', $_GET)) {
    if ($_GET["action"] == 'approve') {
        DBPostgres::getOrderHandler()->accept($_COOKIE['PROFILE_ID'], $_GET['id']);
    } elseif ($_GET["action"] == 'reject') {
        DBPostgres::getOrderHandler()->reject($_COOKIE['PROFILE_ID'], $_GET['id']);
        ...
        try {
            ...
            $index = array_search($_COOKIE['PROFILE_ID'], $table);
        ...
        DBPostgres::getOrderHandler()->finish($_COOKIE['PROFILE_ID'], $_GET['id']);
    }
}
?>
<div class='main'>
    ...
    <div class="list">
        <?php
        $newOrders = DBPostgres::getOrderHandler()->getNewByMaster($_COOKIE['PROFILE_ID']);
        foreach ($newOrders as $order) { ?>
            <div class="item">
                <div class='order'>
                    <a href="/~s338923/isbd/orders.php?action=approve&id=<?= $order->getId(); ?>">Принять заказ</a>
                    <a href="/~s338923/isbd/orders.php?action=reject&id=<?= $order->getId(); ?>&date=<?= $order->getDate(); ?>">Отклонить
                        заказ</a>
    ...
    <div class="list">
        <?php
        $currentOrders = DBPostgres::getOrderHandler()->getCurrentByMaster($_COOKIE['PROFILE_ID']);
    ...
    <div class="list">
        <?php
        $oldOrders = DBPostgres::getOrderHandler()->getHistoryByMaster($_COOKIE['PROFILE_ID']);
